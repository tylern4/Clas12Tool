...
void reader::open(const char *filename) {
  ...
  if (isRandomAccess == true) {
    ...
    // printf(" read record = %12ld next %12ld  nevenets = %d length
    // =%d\n",sequence.getPosition(),sequence.getNextPosition(),sequence.getRecordEvents(),
    // length);
  }
...
void reader::readHeader() {
  ...
  header.firstRecordPosition =
      4 * header.headerLength + header.userHeaderLength;
...
bool reader::next() {
  ...
  if (isRandomAccess == true) {
    ...
    inRecordStream.readHipoEvent(inEventStream,
                                 inReaderIndex.getRecordEventNumber());
  } else {
    // int current_event = sequence.getCurrentEvent();
    // printf("next() : current event %d has event
    // %d\n",current_event,sequence.hasEvents());
    if (sequence.hasEvents() == false) {
      // printf(" READING NEXT BANCH \n");
      if (sequence.getNextPosition() < 0) {
        // printf("**** reached the end of file : records = %ld , events =
      ...
      bool status = inRecordStream.readRecord(inputStream, positionOffset, 0,
                                              inputStreamSize);
      recordsProcessed++;
      if (status == false) {
        // printf("**** reached the end of file : records = %ld , events =
...
void reader::showInfo() {
  ...
  long recordPosition =
      1000; // recordIndex[recordIndex.size()-1].recordPosition;
...
void reader::printWarning() {
  ...
  std::cout << "* WARNING:                                           *"
            << std::endl;
  std::cout << "*   This library war compiled without LZ4 support.   *"
            << std::endl;
  std::cout << "*   Reading and writing compressed buffers will not  *"
            << std::endl;
  std::cout << "*   work. However un-compressed file I/O will work.  *"
            << std::endl;
  std::cout << "******************************************************"
            << std::endl;
#endif
