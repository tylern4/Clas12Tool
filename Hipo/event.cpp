...
std::vector<int> event::getInt(int group, int item) {
  ...
  if (position >= 0) {
    ...
    if (type == 2) {
      ...
      for (int i = 0; i < iter; i++) {
        int16_t *ptr =
            reinterpret_cast<int16_t *>(&dataBuffer[position + 8 + i * 2]);
        vector.push_back((int)*ptr);
      }
    }
...
std::vector<float> event::getFloat(int group, int item) {
  ...
  if (position >= 0) {
    ...
    if (type == 4) {
      ...
      for (int i = 0; i < iter; i++) {
        float *ptr =
            reinterpret_cast<float *>(&dataBuffer[position + 8 + i * 4]);
    ...
    if (type == 5) {
      ...
      for (int i = 0; i < iter; i++) {
        double *ptr =
            reinterpret_cast<double *>(&dataBuffer[position + 8 + i * 8]);
...
void event::scanEvent() {
  ...
  while (position + 8 < eventSize) {
    ...
    int length = *(reinterpret_cast<int *>(&dataBuffer[position + 4]));
    // printf("group = %4d , item = %4d\n",(unsigned int) gid, (unsigned int)
    // iid); if(gid==group&&iid==item) return position;
