...
int schema::getMaxStringLength() {
  int length = 0;
  for (std::map<std::string, std::pair<int, int>>::iterator it =
           schemaEntries.begin();
...
std::vector<std::string> schema::getEntryList() {
  ...
  it=schemaEntries.begin(); it!=schemaEntries.end(); ++it){
...
std::vector<std::string> schema::getRootBranchesCode() {
  ...
  for (std::map<std::string, std::pair<int, int>>::iterator it =
    ...
    sprintf(c_line, "   std::vector<%s>   vec_%s;", rootType.c_str(),
            branch.c_str());
    // node.append(c_line);
    code.push_back(node);
  ...
  sprintf(c_name, "   if(dictionary->hasSchema(\"%s\")==true){",
          scname.c_str());
  ...
  for (std::map<std::string, std::pair<int, int>>::iterator it =
    ...
    sprintf(c_line, "         tree->Branch(\"%s\",&vec_%s);", branch.c_str(),
            branch.c_str());
    code.push_back(c_line);
...
std::vector<std::string> schema::getRootFillCode() {
  ...
  for (std::map<std::string, std::pair<int, int>>::iterator it =
    ...
    code.push_back(c_line);
    sprintf(c_line,
            "         memcpy(&vec_%s[0],%s->getAddress(),nsize*sizeof(%s));",
...
std::vector<std::string> schema::branchesCode() {
  ...
  for (std::map<std::string, std::pair<int, int>>::iterator it =
    ...
    sprintf(c_name, "*%s_%s", scname.c_str(), it->first.c_str());
    std::string branch =
        getBranchVariable(c_name, maxLength + scname.size() + 2);
    /*node.append(scname);
    node.append("_");
  ...
  while (status) {
    ...
    if (item.size() < 2) {
      ...
      if (type != 0) {
        ...
        // printf("** error ** entry %s int schema %s has undefined
