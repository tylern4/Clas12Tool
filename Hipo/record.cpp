...
void record::readRecord(std::ifstream &stream, long position, int dataOffset) {
  ...
  recordHeader.recordLength =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[0]));
  recordHeader.headerLength =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[8]));
  recordHeader.numberOfEvents =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[12]));
  recordHeader.bitInfo = *(reinterpret_cast<int *>(&recordHeaderBuffer[20]));
  recordHeader.signatureString =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[28]));
  recordHeader.recordDataLength =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[32]));
  recordHeader.userHeaderLength =
      *(reinterpret_cast<int *>(&recordHeaderBuffer[24]));
  ...
  if (recordHeader.signatureString == 0x0001dac0) {
    ...
    recordHeader.numberOfEvents =
        __builtin_bswap32(recordHeader.numberOfEvents);
    recordHeader.recordDataLength =
        __builtin_bswap32(recordHeader.recordDataLength);
    recordHeader.userHeaderLength =
        __builtin_bswap32(recordHeader.userHeaderLength);
  ...
  recordHeader.userHeaderLengthPadding =
      (recordHeader.bitInfo >> 20) & 0x00000003;
  ...
  /*printf(" allocating buffer for record, size = %d, padding = %d length = %d
    type = %d nevents = %d data length = %d\n", dataBufferLengthBytes,
    compressedDataLengthPadding, recordHeader.recordDataLengthCompressed*4,
    recordHeader.compressionType, recordHeader.numberOfEvents,
    recordHeader.recordDataLength);
    */
  ...
  if (dataBufferLengthBytes > recordCompressedBuffer.size()) {
    ...
    // printf("---> resizing internal compressed buffer size to from %ld to
  ...
  int decompressedLength =
      recordHeader.indexDataLength + recordHeader.userHeaderLength +
  ...
  // compressedDataLengthPadding); showBuffer(&recordBuffer[0], 10, 200);
  if (recordHeader.compressionType == 0) {
    // printf("compression type = 0 data length = %d\n", decompressedLength);
    memcpy((&recordBuffer[0]), (&recordCompressedBuffer[0]),
           decompressedLength);
  } else {
    int unc_result =
...
bool record::readRecord(std::ifstream &stream, long position, int dataOffset,
                        long inputSize) {
  ...
  stream.seekg(dataposition, std::ios::beg);
  // stream.read( compressedBuffer, dataBufferLengthBytes);
  if (position + dataBufferLengthBytes + recordHeader.headerLength >
      inputSize) {
...
void record::readRecord__(std::ifstream &stream, long position,
                          long recordLength) {
  ...
  if (recordLength > recordCompressedBuffer.size()) {
    ...
    // printf("---> resizing internal compressed buffer size to from %ld to
    // %d\n", recordCompressedBuffer.size(), newSize); printf("---> after
    // printout\n");
    recordCompressedBuffer.resize(newSize);
  }
  ...
  // stream.read( (char *) &recordHeaderBuffer[0],80);
  recordHeader.recordLength =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[0]));
  recordHeader.headerLength =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[8]));
  recordHeader.numberOfEvents =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[12]));
  recordHeader.bitInfo =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[20]));
  recordHeader.signatureString =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[28]));
  recordHeader.recordDataLength =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[32]));
  recordHeader.userHeaderLength =
      *(reinterpret_cast<int *>(&recordCompressedBuffer[24]));
...
void record::getData(hipo::data &data, int index) {
  ...
  if (index > 0) {
    first_position =
        *(reinterpret_cast<uint32_t *>(&recordBuffer[(index - 1) * 4]));
  }
...
#ifdef __LZ4__
  int result =
      LZ4_decompress_safe(data, dest, dataLength, dataLengthUncompressed);
